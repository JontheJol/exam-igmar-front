...
export class EditComentarioComponent implements OnInit {
  ...
  obtenerComentario(comentarioId: number): void {
    const token = this.cookieService.get('authToken');
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    const endpoint = `https://6f6f-2806-101e-d-a299-c169-f1b5-8ce1-acf5.ngrok-free.app/api/comments/${comentarioId}`;
  ...
  initializeForm(): void {
    if (this.comentario) {
      ...
      const token = this.cookieService.get('authToken');
      const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
      this.http.get<any>(`https://6f6f-2806-101e-d-a299-c169-f1b5-8ce1-acf5.ngrok-free.app/api/users/${this.comentario.user_id}`, {headers: headers}).subscribe(
        (userData: any) => {
  ...
  onSubmit(): void {
    if (this.comentarioForm.valid && this.comentario && this.comentario.product_id) {
      ...
      const endpoint = `https://6f6f-2806-101e-d-a299-c169-f1b5-8ce1-acf5.ngrok-free.app/api/comments/${comentarioId}/update`;
      const userData = {
